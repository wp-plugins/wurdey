...
{   
    ...
    static function Instance() {
        if (WurdeyChildWordfence::$instance == null) {
            WurdeyChildWordfence::$instance = new WurdeyChildWordfence();
        }
        return WurdeyChildWordfence::$instance;
    }    
    
    
    public function __construct() {
        add_action('wurdey_child_deactivation', array($this, 'deactivation'));
    }
    
    public function deactivation()
    {
        if ($sched = wp_next_scheduled('wurdey_child_wordfence_cron_scan')) {
            wp_unschedule_event($sched, 'wurdey_child_wordfence_cron_scan');
        }
    }
    ...
    function set_showhide() {
        WurdeyHelper::update_option('wurdey_wordfence_ext_enabled', "Y");
        $hide = isset($_POST['showhide']) && ($_POST['showhide'] === "hide") ? 'hide' : "";
        WurdeyHelper::update_option('wurdey_wordfence_hide_plugin', $hide);
        $information['result'] = 'SUCCESS';
        return $information;
    }
    public function wordfence_init()
    {  
        if (get_option('wurdey_wordfence_ext_enabled') !== "Y")
            return;
        
        if (get_option('wurdey_wordfence_hide_plugin') === "hide")
        {
    ...
    public function init_cron() {                       
        $sched = wp_next_scheduled('wurdey_child_wordfence_cron_scan');
        $sch = get_option("wurdey_child_wordfence_cron_time");
        if ($sch == "twicedaily" ||
            $sch == "daily" ||
            $sch == "weekly" ||
            $sch == "monthly") {
            add_action('wurdey_child_wordfence_cron_scan', array($this, 'wfc_cron_scan'));
            if ($sched == false)
            {   
                $sched = wp_schedule_event(time(), $sch, 'wurdey_child_wordfence_cron_scan');
            }  
        } else {
            if ($sched != false) {
                wp_unschedule_event($sched, 'wurdey_child_wordfence_cron_scan');
	...
	function restore_file(){
		...
		if($opts['autoUpdate'] == '1'){
			...
			wfConfig::disableAutoUpdate();
                ...
                if ($sch != get_option('wurdey_child_wordfence_cron_time')) {
                    update_option('wurdey_child_wordfence_cron_time', $sch);
                    $sched = wp_next_scheduled('wurdey_child_wordfence_cron_scan');
                    if ($sched != false) {
                        wp_unschedule_event($sched, 'wurdey_child_wordfence_cron_scan');
                    }
