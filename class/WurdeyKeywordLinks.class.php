...
{   
    ...
    static function Instance() {
        if (WurdeyKeywordLinks::$instance == null) {
            WurdeyKeywordLinks::$instance = new WurdeyKeywordLinks();
        }
        return WurdeyKeywordLinks::$instance;
    }
    
    public function __construct() {
        global $wpdb;
        $this->server = get_option('wurdey_child_server');
        ...
        $this->config = get_option('wurdey_kwl_options', array());
        $this->keyword_links = get_option('wurdey_kwl_keyword_links', array());
    ...
    {
        $dell_all = array('wurdey_keyword_links_htaccess_set',
                        'wurdey_kwl_options',
                        'wurdeyKeywordLinks',
                        'wurdey_kwl_keyword_links',
                        'wurdey_kwl_click_statistic_data',
                        'wurdey_kwl_enable_statistic',
                        'wurdey_kwl_keyword_links');
    ...
    {	
        if (!is_admin() && get_option('wurdey_kwl_enable_statistic'))
        {                
      ...
      public function permalinkChanged($old_struct, $new_struct)
    {
        if (get_option('wurdeyKeywordLinks') != 1) {
            if (get_option('wurdey_keyword_links_htaccess_set') == 'yes') {
    ...
    {      
        if ($force_clear)
            ...
            if ('' ==  get_option( 'permalink_structure') && get_option('wurdey_keyword_links_htaccess_set') != 'yes')
                $this->do_update_htaccess(); // need to update
            else if ('' !=  get_option( 'permalink_structure') && get_option('wurdey_keyword_links_htaccess_set') == 'yes')
                $this->do_update_htaccess(); // need to update
        }
        return true;
    }
    ...
    public static function clear_htaccess() {
        ...
        {
            $rules = explode("\n", '');
            insert_with_markers($htaccess_file, 'Wurdey Keyword Links Extension', $rules);
        }
        WurdeyHelper::update_option('wurdey_keyword_links_htaccess_set', '');
    }
    ...
    public function do_update_htaccess($force_clear = false) {    
        if ($force_clear) {
            ...
            WurdeyHelper::update_option('wurdey_keyword_links_htaccess_set', 'yes');
            return true;
        } else {
            self::clear_htaccess();
            return true;
    ...
    public function sendClick()
    {
            ...
            $clickData = get_option('wurdey_kwl_click_statistic_data');
            $key = get_option('wurdey_child_pubkey');
            ...
            if ( is_array($request) && intval($request['body']) > 0 )
                    delete_option('wurdey_kwl_click_statistic_data');
    }
    ...
    public function set_option($key, $value) {
        ...
        return WurdeyHelper::update_option('wurdey_kwl_options', $this->config);
    }
    ...
    public function set_link($link_id, $link) {
        ...
        return WurdeyHelper::update_option('wurdey_kwl_keyword_links', $this->keyword_links);
    }
    ...
    public function filter_content($content) {
        global $post, $wpdb;        
        if ($this->get_option('wurdey_kwl_do_not_link_site_blocked', false))
            return $content;
        ...
        if ($post) {            
            ...
            $disable = get_post_meta($post->ID, '_wurdey_kl_disable', true);
            if ($disable == 1)
                return $content;      
            
            $paths_blocked = $this->get_option('wurdey_kwl_do_not_link_paths_blocked', array());
        ...
        // save specific link 
        if ($post) {            
            $specific_link = unserialize(get_post_meta($post->ID, '_wurdey_kwl_specific_link', true));
            if (is_array($specific_link) && count($specific_link) > 0) {  
                $specific_link = current($specific_link);
                $specific_link->post_id = $post->ID;                
                //update_post_meta($post->ID, '_wurdey_kwl_specific_link_save', array($specific_link->id => $specific_link));
                update_post_meta($post->ID, '_wurdey_kwl_specific_link_id', $specific_link->id);
                if ($this->set_link($specific_link->id, $specific_link))                                
                    update_post_meta($post->ID, '_wurdey_kwl_specific_link', "<saved>");
            }                         
        }
...
//        }        
        ...
        $not_allow_keywords = get_post_meta($post->ID, 'wurdey_kl_not_allowed_keywords_on_this_post', true);
        ...
        //$content = preg_replace_callback('/\{WUURDEY_LINK +HREF="(.*?)" +TARGET="(.*?)" +REL="(.*?)" +LINK-ID="(.*?)" +CLASS="(.*?)" +TEXT="(.*?)" *\}/is', array(&$this, 'keyword_replace'), $content);
        $content = preg_replace_callback('/\{WUURDEY_LINK +HREF="(.*?)" +TARGET="(.*?)" +REL="(.*?)" +LINK-ID="(.*?)" +CLASS="(.*?)" +TEXT="(.*?)" +FULL_TEXT="(.*?)" *\}/is', array(&$this, 'keyword_replace'), $content);
...
////            }
//            $link_target = get_post_meta($this->link_temp->id, '_wurdey_kl_link_newtab', true);
//            $this->link_temp->link_target = ( $link_target != -1 && $link_target == 1 ? '_blank' : '' );
//            $link_rel = get_post_meta($this->link_temp->id, '_wurdey_kl_link_nofollow', true);
//            $this->link_temp->link_rel = ( $link_rel != -1 && $link_rel == 1 ? 'nofollow' : '' );
//            $this->link_temp->link_class = get_post_meta($this->link_temp->id, '_wurdey_kl_link_class', true);
//        }
        ...
        //return '{WUURDEY_LINK HREF="' . ( $this->link_temp->cloak_path ? $this->siteurl . $redirection_folder . '/' . $this->link_temp->cloak_path : $this->link_temp->destination_url) . '" TARGET="' . $target . '" REL="' . $rel . '" LINK-ID="' . (isset($this->link_temp->id) ? $this->link_temp->id : 0) . '" CLASS="' . $class . '" TEXT="' . $matches[1] . '"}';
        return '{WUURDEY_LINK HREF="' . ( $this->link_temp->cloak_path ? $this->siteurl . $redirection_folder . '/' . $this->link_temp->cloak_path : $this->link_temp->destination_url) . '" TARGET="' . $target . '" REL="' . $rel . '" LINK-ID="' . (isset($this->link_temp->id) ? $this->link_temp->id : 0) . '" CLASS="' . $class . '" TEXT="' . $matches[2] . '" FULL_TEXT="' . $matches[1] . '"}';
    }
...
//            return $links;
        ...
        if ($post && $post->ID > 0) 
            $spec_link_id = get_post_meta($post->ID, '_wurdey_kwl_specific_link_id', true);
     ...
     public function get_post_keywords($post_type, $cats = null) {
        ...
        //$results = $wpdb->get_results(sprintf("SELECT * FROM $wpdb->posts as p LEFT JOIN $wpdb->postmeta as pm ON p.ID=pm.post_id $join WHERE p.post_status='publish' AND p.post_type='%s' AND pm.meta_key='_wurdey_kl_post_keyword' $where", $post_type));
    ...
    public function redirect_cloak() {
        ...
        if (!empty($destination_url)){
            if (get_option('wurdey_kwl_enable_statistic'))
    ...
    public function add_statistic($link_id, $addr, $referer, $type = 'click') {
        if ($link_id > 0) {
            $storeData = get_option('wurdey_kwl_click_statistic_data');
            ...
            WurdeyHelper::update_option('wurdey_kwl_click_statistic_data', $storeData);
...
//        if ($link_id) {
//            $stat_data = get_option('wurdey_kwl_statistic_data_' . $link_id, array());
//            if ($stat_data) {
//                $return['stat_data'] = $stat_data;                
//                //$wpdb->query("UPDATE {$wpdb->prefix}options SET option_name = 'wurdey_kwl_statistic_data_done_" . $link_id . "' WHERE option_name = 'wurdey_kwl_statistic_data_" . $link_id . "'");
//                update_option('wurdey_kwl_statistic_data_' . $link_id, '');
//            } else 
...
//    }
    ...
    function remove_keywords() {
        ...
        if (is_array($remove_kws) && is_array($this->keyword_links)) {
            ...
            WurdeyHelper::update_option('wurdey_kwl_keyword_links', $this->keyword_links);
            $return['status'] = 'SUCCESS';
        } else {      
    ...
    public function enable_stats()
    {
        ...
        if (WurdeyHelper::update_option('wurdey_kwl_enable_statistic', $enable_stats))
            $return['status'] = 'SUCCESS';                      
        return $return;
    }
    ...
    public function refresh_data()
    {
        ...
        if (isset($_POST['clear_all'])) {
            $cleared1 = WurdeyHelper::update_option('wurdey_kwl_keyword_links', '');
            $cleared2 = WurdeyHelper::update_option('wurdey_kwl_options', '');
            ...
            if ($current) {
                if ($delete_permanent) {
                    if ($current->type == 2 || $current->type == 3)                     
                        $deleted = delete_post_meta($current->post_id, '_wurdey_kwl_specific_link_id');
    ...
    public function edit_link() {
        ...
        if (!empty($link_id)) {
                ...
                $link->case_sensitive = intval($_POST['case_sensitive']);  
                 ...
                 if ($link->type == 2 || $link->type == 3) {
                    ...
                    if ($link->post_id) {
                         update_post_meta($link->post_id, '_wurdey_kwl_specific_link_id', $link_id);
                    }
                } 
        ...
        WurdeyHelper::update_option('wurdeyKeywordLinks', 1); // enable extension functions
            return $return;
    }
    ...
    public function update_config() {
        ...
        WurdeyHelper::update_option('wurdeyKeywordLinks', 1); // enable extension functions
            if (WurdeyHelper::update_option('wurdey_kwl_options', $this->config)) {
                $return['status'] = 'SUCCESS';    
			}
    ...
    {
        $return = array();
        if ($this->set_option('wurdey_kwl_do_not_link_site_blocked', true))
            $return['status'] = 'SUCCESS';
        return $return;            
    }    
    ...
    public function donotlink_path_blocks()
    {
        $return = array();
        if ($path = $_POST['path']) {
            $paths = $this->get_option('wurdey_kwl_do_not_link_paths_blocked', array());
            $paths[] = $path; 
            if ($this->set_option('wurdey_kwl_do_not_link_paths_blocked', $paths))
                $return['status'] = 'SUCCESS';
        }
        return $return;            
    }    
    ...
    {
        $return = array();
        if ($this->set_option('wurdey_kwl_do_not_link_site_blocked', ''))
            $return['status'] = 'SUCCESS';
        if ($this->set_option('wurdey_kwl_do_not_link_paths_blocked', ''))
            $return['status'] = 'SUCCESS';
        return $return;            
    }    
