...
//        return function_exists('system');
    ...
    {
        if (!$this->file_exists(WP_CONTENT_PATH.'/')) throw new Exception(__('Not a full backup.','wurdey-child'));
        if (!$this->file_exists('wp-admin/')) throw new Exception(__('Not a full backup.','wurdey-child'));
        if (!$this->file_exists(WP_CONTENT_PATH.'/dbBackup.sql')) throw new Exception(__('Database backup not found.','wurdey-child'));
    }
    ...
    public function readConfigurationFile()
    {
        ...
        if ($configContents === FALSE) throw new Exception(__('Cant read configuration file from backup','wurdey-child'));
        $this->config = unserialize(base64_decode($configContents));

        if (isset($this->config['plugins'])) WurdeyHelper::update_option('wurdey_temp_clone_plugins', $this->config['plugins']);
        if (isset($this->config['themes'])) WurdeyHelper::update_option('wurdey_temp_clone_themes', $this->config['themes']);
    }
    ...
    {
        $link = @WurdeyChildDB::connect($this->config['dbHost'], $this->config['dbUser'], $this->config['dbPass']);
        if (!$link) throw new Exception(__('Invalid database host or user/password.','wurdey-child'));

        $db_selected = @WurdeyChildDB::select_db($this->config['dbName'], $link);
        if (!$db_selected) throw new Exception(__('Invalid database name','wurdey-child'));
    }
    ...
    public function update_option($name, $value)
    {
        ...
        if ($var == NULL)
        {
            $wpdb->query('INSERT INTO '.$this->config['prefix'].'options (`option_name`, `option_value`) VALUES ("'.$name.'", "'.WurdeyChildDB::real_escape_string(maybe_serialize($value)).'")');
        }
        else
        {
            $wpdb->query('UPDATE '.$this->config['prefix'].'options SET option_value = "'.WurdeyChildDB::real_escape_string(maybe_serialize($value)).'" WHERE option_name = "'.$name.'"');
        }
    }
...
////                $query = str_replace("\\\\'", "\\'", $query);
////                $query = str_replace('\r\n', '\\\r\\\n', $query);
//                if ($wpdb->query($query) === false) throw new Exception(__('Error importing database','wurdey-child'));
//            }
//
            if (!feof($handle))
            {
                throw new Exception(__('Error: unexpected end of file for database','wurdey-child'));
            }
            fclose($handle);
...
//            $wpdb->query('UPDATE '.$table_prefix.'options SET option_value = "'.WurdeyChildDB::real_escape_string($option_val).'" WHERE option_id = '.$option_id);
//        }
    ...
    function icit_srdb_replacer( $connection, $search = '', $replace = '', $tables = array( ) ) {
        ...
        if ( is_array( $tables ) && ! empty( $tables ) ) {
            foreach( $tables as $table ) {
                ...
                $fields = WurdeyChildDB::_query( 'DESCRIBE ' . $table, $connection );
                while( $column = WurdeyChildDB::fetch_array( $fields ) )
                    $columns[ $column[ 'Field' ] ] = $column[ 'Key' ] == 'PRI' ? true : false;
				
                // Count the number of rows we have in the table if large we'll split into blocks, This is a mod from Simon Wheatley
                $row_count = WurdeyChildDB::_query( 'SELECT COUNT(*) as count FROM ' . $table, $connection ); // to fix bug
                $rows_result = WurdeyChildDB::fetch_array( $row_count );
                ...
                for( $page = 0; $page < $pages; $page++ ) {					
                    ...
                    $data = WurdeyChildDB::_query( sprintf( 'SELECT * FROM %s LIMIT %d, %d', $table, $start, $end ), $connection );
                    if ( ! $data )
                        $report[ 'errors' ][] = WurdeyChildDB::error( );

                    while ( $row = WurdeyChildDB::fetch_array( $data ) ) {
                        ...
                        foreach( $columns as $column => $primary_key ) {
                            ...
                            if ( $edited_data != $data_to_fix ) {								
                                $report[ 'change' ]++;											
                                $update_sql[] = $column . ' = "' . WurdeyChildDB::real_escape_string( $edited_data ) . '"';
                                $upd = true;
                            }

                            if ( $primary_key )
                                $where_sql[] = $column . ' = "' . WurdeyChildDB::real_escape_string( $data_to_fix ) . '"';
                        }

                        if ( $upd && ! empty( $where_sql ) ) {
                            $sql = 'UPDATE ' . $table . ' SET ' . implode( ', ', $update_sql ) . ' WHERE ' . implode( ' AND ', array_filter( $where_sql ) );							
                            $result = WurdeyChildDB::_query( $sql, $connection );
                            if ( ! $result )
                                $report[ 'errors' ][] = WurdeyChildDB::error( );
                            else
                                $report[ 'updates' ]++;
